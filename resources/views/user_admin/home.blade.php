                            ...
                            @foreach($subshares as $val)
                                <div class="inner-feed-area">
                                    <ul>
                                        ...
                                        <li>
                                        <input type="hidden" id="user_imageold_subshare_{{$val->sub_id}}" value="{{ \Helpers::avatarURL($val->image)}}">
                                            ...
                                            <input type="hidden" id="track_insold_subshare_{{$val->sub_id}}" value="{{$val->custom_agreement}}">
                                            <input type="hidden" id="track_perold_subshare_{{$val->sub_id}}" value="{{$val->percentage}}">
                                            <input type="hidden" id="track_statusold_{{$val->sub_id}}" value="{{$val->status}}">
                            ...
                            <div id="menu1" class="tab-pane fade">
                                ...
                                <div class="inner-feed-area">
                                    <ul>
                                        ...
                                        <li><p>{{ \Helpers::get_time_ago(strtotime($val->created_at)) }}</p></li>
                                        <li>
                                       <input type="hidden" id="user_imageold_offers_{{$val->sub_id}}" value="{{ \Helpers::avatarURL($val->image)}}">
                                            ...
                                            <input type="hidden" id="track_insold_offers_{{$val->sub_id}}" value="{{$val->custom_agreement}}">
                                            <input type="hidden" id="track_perold_offers_{{$val->sub_id}}" value="{{$val->percentage}}">
                                            <input type="hidden" id="track_statusold_{{$val->sub_id}}" value="{{$val->status}}">
                            ...
                            <div id="menu2" class="tab-pane fade active in">
                                ...
                                <div class="inner-feed-area">
                                    <ul>
                                        ...
                                        <li><p>{{ \Helpers::get_time_ago(strtotime($val->created_at)) }}</p></li>
                                        <li>
                                        <input type="hidden" id="user_imageold_response_{{$val->sub_id}}" value="{{ \Helpers::avatarURL($val->image)}}">
                                            ...
                                            <input type="hidden" id="track_insold_response_{{$val->sub_id}}" value="{{$val->custom_agreement}}">
                                            <input type="hidden" id="track_perold_response_{{$val->sub_id}}" value="{{$val->percentage}}">
                                            <input type="hidden" id="track_statusold_{{$val->sub_id}}" value="{{$val->status}}">
...
?>
      ...
      <div class="white-small-tabs" style="background: none; margin-top: 0;height: 600px;overflow-y: scroll;padding: 0px;" id="facebook-posts">
        ...
        <!-- loop facebook user posts start -->
        @foreach($post as $row)
...
var wavesurfer1="";
           ...
           $("#userimg").html('<img src="'+$("#user_imageold_"+prefix+"_"+id).val()+'" style="width: 50px;height: 50px;border-radius: 50%;">');
             ...
             $("#track_ins").html($("#track_insold_"+prefix+"_"+id).val());
              ...
              $.ajaxSetup({
               ...
               if($("#track_statusold_"+sub_id).val() != 'active'){
         $("#acc_rej_btn").html('<button type="button" class="btn btn-default my_btn" onclick="accept_offer('+sub_id+')">Accept the Offer</button><button type="button" class="btn btn-default my_btn" data-dismiss="modal">Discuss Offer</button>');
      }else{
          $("#acc_rej_btn").html("");
      }
       }
   function accept_offer(id){
        ...
        $.ajax({
            ...
            success: function(data)
            {
               window.location.replace('{{url("/subshares")}}');
            }
        });
    }
</script>
