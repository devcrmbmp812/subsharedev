...
<aside class="right-side">
		<ol class="breadcrumb pad">
			...
			<li class="active"><a href="#"><i class="fa fa-angle-right" aria-hidden="true"></i>Step: 4</a></li>
		</ol>
        <section class="subhare-home home-content content three_a process">
            <div class="">
                ...
                <div class="col-md-6 last-sub-div">
                    ...
                    <?php 
                     $new_one = DB::table('subshare_tracks')->leftjoin('uploads','uploads.id','=','subshare_tracks.resubmit_id')->where('subshare_id',Request::segment(2))->first();
                    ?>
					<div class="sub-detail-area extra green">
						<div class="col-md-12">
							...
							<div class="wave-graph" style="width: 100%;" >
                                                        ...
                                                        <a class="btn btn-info btn-lg small-green btn_custom" target="_blank" href="{{ url('/server/php/files/').'/'.$new_one->producer_new_track}}" download=""> Download</a>
						...
						<div class="col-md-12">
							<h4>New Track 1</h4>
                                                    @if( \Helpers::SubshareUploaderID(Request::segment(2)) == Auth::user()->id && !$new_one->producer_new_track1)
                                                            ...
                                                            <div class="submit-top-area">
                                                                <div class=" dz-default dz-message">
                                                                  <span id="style_btn" style="background: #eff3f6;border: 0px;border-radius: 0px;padding: 10px;margin-bottom: 10px;" class="btn btn-default fileinput-button">
                                                                       ...
                                                                       <input id="fileupload_new1" accept="audio/*" type="file" name="files[]" multiple="">
                                                                  </span>
                                                    ...
                                                    @endif
                                                    @if($new_one->producer_new_track1)
                                                       <span class="time">{{\Helpers::get_time_ago(strtotime($subshare_tracks->created_at))}}</span>
							<div class="clearfix"></div>
							<div class="wave-graph" style="width: 100%;" >
                                                            <div id="wave_new1<?php echo $new_one->id; ?>"></div>
                                                            <img style="height: 100px; width:100%;" id="playerimg_new1{{$new_one->id}}"  src="{{ url('assets/img/wave%20copy.png') }}">
                                                            <a href="javascript:void(0)" id="start_new1{{$new_one->id}}" onclick="loadPlayernew1('{{ url('server/php/files/'. $new_one->producer_new_track1) }}', '{{ $new_one->id}}' );"><img src="{{ url('assets/img/Play%20button.png') }}"></a>
                                                            <a href="javascript:void(0)" class="play" id="play-new1{{ $new_one->id }}" style="display: none;" data-id='{{ $new_one->id }}'>
                                                                <img class="img12" src="{{url('assets/img/player-play-button.png')}}">
                                                                <img class="img13" src="{{url('assets/img/player-pause-button.png')}}" style="display:none;">
                                                            </a>
                                                        </div>
                                                        <a class="btn btn-info btn-lg small-green btn_custom" target="_blank" href="{{ url('/server/php/files/').'/'.$new_one->producer_new_track1}}" download=""> Download</a>
				...
				<div class="row">
                                    ...
                                    <div class="row adv-player-bottom-area" style="width: 100% !important; position: relative; margin-top: 50px;">
                                                        <div class="col-md-12">
                                                            <div>
                                                                <div class="creation-area shareprocess" style="">
                                                                          @if($new_one->resubmit_id)
                                                                         <div class="wave-graph">
                                                                                 <div id="waveformjs"></div>

                                                                                        <a href="javascript:void(0)" class="sub-anchor" onclick="customplayPausejs()" id="play-{{ $lastSubshare->id }}" data-id='{{ $lastSubshare->id }}'>
                                                                                            <img class="img9" src="{{ url('assets/img/player-play-button.png') }}" class="img-responsive sub-two-play">
                                                                                            <img class="img10" src="{{ url('assets/img/player-pause-button.png') }}" style="display: none">
                                                                                        </a>
                                                                          </div>
                                                                          @endif
                                                                          @if(!$new_one->resubmit_id)
                                                                          <h4>No Re-Submitted Track exists</h4>
                                                                           <?php
                                                                        $subshare_userid = \Helpers::SubshareUploaderID(Request::segment(2));
                                                                        if($subshare_userid == Auth::user()->id){?>  
                                                                                   <div class="sub_athree" style="float: none;position: relative;text-align: center;">
                                                                                        <button type="button" class="btn btn-success"  onclick="window.location.replace('{{url("/track?resub=").Request::segment(2)}}')">Re-Submit Track</button>
                                                                                    </div>
                                                                            <?php }?>
                                                                          @endif
				...
				<div class="col-md-11">
                                 <div class="sub_athree"><button type="button" class="btn btn-success" onclick="window.location.replace('{{url('/subshare_laststep/').'/'.Request::segment(2) }}')"><img class="align-left" src="../assets/images/icon-submit.png"> <span>Submit</span></button></div>
				</div>
                           </div> 
                <!-- col-md-6-end -->
            </div>
...
<script>
    var wavesurferjs = WaveSurfer.create({
        container: '#waveformjs',
        waveColor: 'gray',
        progressColor: 'red',
        audioRate: 1,
        barWidth: 3,
      ...
      wavesurferjs.load("{{ url('server/php/files/' . $new_one->file_name )}}");
      /* Update labels */
...
wavesurferjs.on('region-created', updateLabel);
wavesurferjs.on('region-updated', updateLabel);
function customplayPausejs()
        {
            var cuurent_id= $(this).attr('id');
            wavesurferjs.playPause();

            if(wavesurferjs.isPlaying() == true)
            {
                $('.img10').show();
                $('.img9').hide();
            }else {
            ...
            wavesurferjs.on('finish', function () {
                $(' .img9').show(); // display play icon.
                $(' .img10').hide();
            });
        }
        ...
        var region = wavesurferjs.addRegion({
          id:region_id,
          start:start_time,
          end:end_time,
          drag:false,
...
function play_rigion(strat_time,end_time){
  ...
  wavesurferjs.play(strat_time,end_time);

  $('.img10').show();
  $('.img9').hide();


  wavesurferjs.on('pause', function () {
    $('.img9').show();
    $('.img10').hide();
  });
...
var tags = [];
  ...
  <?php
$tags = DB::table('audio_tags')->where('audio_id',$new_one->id)->get();
foreach($tags as $val){?>
...
<script>
    wavesurferjs.on('ready',function(){
...
$('body').on('keyup',function(e){
 var cuurent_id= 'play-new'+upload_id;
  ...
  // load player/
   function loadPlayernew1(file_name, upload_id )
   {
    var wavesurfern = Object.create(WaveSurfer);
    wavesurfern.init({
        container: '#wave_new1'+upload_id,
        progressColor: 'red',
        audioRate: 1,
        barWidth: 3,
        height: 150,
    ...
    $("#playerimg_new1"+upload_id).hide();
    $("#start_new1"+upload_id).hide();
    $("#play-new1"+upload_id).show();
    wavesurfern.load(file_name);
    $('#wave_new1'+upload_id) .siblings('.play').bind('click', function () {
        var cuurent_id= 'play-new1'+upload_id;
        wavesurfern.playPause();
        if(wavesurfern.isPlaying() == true)
        {
            $('#'+cuurent_id+' .img13').show();
            $('#'+cuurent_id+' .img12').hide();
        }else {
            $('#'+cuurent_id+' .img12').show();
            $('#'+cuurent_id+' .img13').hide();
        }
    });
    wavesurfern.on('finish', function () {
      $('#play-new'+upload_id+' .img12').show();
      $('#play-new'+upload_id+' .img13').hide();

  });
    wavesurfern.on('ready', function () {
       wavesurfern.play();
       $('#play-new'+upload_id+' .img13').show();
       $('#play-new'+upload_id+' .img12').hide();
   });
$( "body").unbind( "keyup" );
$('body').on('keyup',function(e){
 var cuurent_id= 'play-new1'+upload_id;
    ...
    $(function () {
        ...
        $('#fileupload_new1').fileupload({
            ...
            async: true,
                ...
                $(".errorMsg").remove();
                $("#fileupload_new1").after("<p class=success>Successfully Uploaded!</p>");
                var duration = 0;
                var name = 0;
                $.each(data.result.files, function (index, file) {
                    ...
                    getDuration('../server/php/files/'+file.name, function(length) {
                        ...
                        $.post('../producerNewtrack',{'_token': '{{csrf_token()}}' ,'name': file.name,'duration': duration,'subshare_id':subshare_id,'producer_new_track1':'1'}, function(data){
